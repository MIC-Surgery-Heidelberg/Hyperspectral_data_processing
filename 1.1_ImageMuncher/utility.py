...
def get_sub_folder_paths(path_to_main_folder):
    ...
    contents = sorted([path_to_main_folder + "/" + i for i in contents if bool(re.match('[\d/-_]+$', i))])
    print(contents)
    return contents
...
def get_sub_folder_content_paths(path_to_sub_folder):
    ...
    res = {}
    contents = sorted(os.listdir(path_to_sub_folder))

    for mode in [RGB,OXY,THI,NIR,TWI,OHI,TLI]:
        if not len(glob.glob(path_to_sub_folder + '/*' + mode + '*.png')) == 0:
            res[mode] = [path_to_sub_folder + "/" + i for i in contents if mode in i][0]
...
def add_background(prs, slide, background_pic):
    left = top = Cm(0)
    pic = slide.shapes.add_picture(background_pic, left, top, width=prs.slide_width, height=prs.slide_height)
